...
事实上，C和C++语言在实践中是有发展出自己的一套信息隐藏和依赖管理机制的，这些做法已经被许多优秀的开源C/C++项目广泛使用着。下面给大家介绍下这些C/C++信息隐藏与依赖管理的优秀实践，虽然和别的语言相比仍旧有不完美的地方，但对于绝大多数项目来说已经完全够用了。
...
上述目录结构是C/C++社区主流的一种布局规范。社区中还有其它的一些布局格式，但是经过对比并不比这个布局清晰及使用范围大。另外这个布局与其它和C/C++语言相似的现代化语言的标准布局是趋于一致的（如RUST）。关于其它各种目录布局的介绍，我们将在后文的优秀开源代码推荐中进一步介绍。
...
4）对头文件的目录管理尽量遵循主流的社区规范；尽量避免将所有模块或者库的对外头文件集中放置到一起然后暴露给所有用户；
```

## 依赖管理

一个项目中，除了非常底层的软件模块，大多数模块都需要别的模块的协助才能完成功能，这需要借助模块之间的依赖管理。
...
首先，我们审视下不同开发阶段对于所依赖的其它模块的可见性需求有哪些。
...
不过对于动态库和可执行程序，不要忘了在最终发布的时候，仍然是需要和真正的依赖方的二进制进行链接的。所以从完整意义上来说，对于动态库和可执行程序，在构建阶段是需要能够获取到依赖方的二进制的。

通过上面的分析我们看到，想要独立开发、构建和测试，最重要的是能够获取依赖方的公开头文件，而在一些场合下还需要依赖方的二进制。有上述这些就够了。

那么回到依赖管理上，**依赖管理就是要保证我们在不同阶段所依赖的东西可以低成本的精准获得，同时又不会过度获得。**
...
说了这么多，那么在基于全量源码的依赖管理方式下，是否就不能做到子模块的独立的开发、构建、测试与发布呢？答案是可以做到。

看看以下做法：

- 每个模块在自己的目录下自行维护自己的公开头文件（还记得前文中推荐过的模块目录布局吗）；
- 每个模块有自己内置的独立构建脚本和启动入口，并且有一致的模块级构建和测试的触发命令；
- 每个模块可以通过构建入口参数或者环境变量获得`PROJECT_ROOT`，作为整个项目源码的根目录；
- 所有模块基于`PROJECT_ROOT`有一套路径命名约定，包括二进制发布路径（可以是每个模块内部的临时目录，也可以是在`PROJECT_ROOT`下的某个集中路径）；
- 每个模块根据自己的实际依赖，在自己的构建里面描述自己所依赖的其它模块的头文件路径（可以按照约定从`PROJECT_ROOT`进行计算），如果还需要其它模块的二进制，就在约定的二进制目录下查找，如果找不到就调用统一的模块构建命令触发依赖模块进行构建；
- 构建成功后，将生成的二进制发布到约定二进制目录中；
- 如果模块要独立发布，需要模块里有内置的打包代码（可以写到构建脚本里面），在构建成功后将自己的头文件和二进制（如果是动态库或者可执行程序，则还要包含所依赖的二进制）按照打包格式进行打包，并发布到对应的仓库中；
- 所有模块构建脚本里的重复代码（如体系架构设定、编译和链接参数、目录计算、打包发布等公共构建代码...），做成每个模块的构建时依赖库，由每个模块的构建脚本自行include调用；

上述实践解决了前面说的头文件和构建耦合问题，最终让每个模块可以做到独立开发、构建、测试和发布。这里每个模块相当于是一个闭包，它自行管理自己的头文件和完整的构建以及命令入口。它依赖了一套约定，这套约定需要大家共同去严格遵守。

仔细看上面的这套约定和做法，会发现它和golang语言的做法有些相似。没错，golang在没有引入module机制之前就是依赖于`GOPATH`的单一代码库管理，它是google的单一代码库实践在golang语言中的应用。

在单一代码库中要做到内部软件模块的独立开发、构建、测试与发布，需要良好的设计规划能力、工程工具能力以及以团队自治为基础但又不缺乏整体协作纪律的组织方式。事实上很多大型C/C++项目采用单一代码库的管理方式，但是难以做到内部的独立构建与发布，和欠缺上述能力是有关的。

采用上述推荐的做法，对构建设计和构建工具的要求都会更高。由于每个模块是一个自治的构建单元，拥有自己独立的构建脚本和内置的构建启动入口，这样所有模块的构建脚本一定会存在重复的配置数据、编译参数和共享的构建代码。这些公共代码可以提炼出来作为共享的构建库，让每个模块构建时自行依赖调用。如果模块采用源码发包的话，这些共享的构建库需要作为包的构建时依赖一起发布。

另外，对构建工具更高的要求，主要是指构建工具要支持构建闭包性。软件模块在构建的时候可能创建了某些构建变量，用于保存编译参数、预编译宏或者依赖的所有头文件路径等，这些构建变量我们希望是闭包于该模块内的。我们不希望在触发了依赖模块的构建时，当前模块的这些构建变量被默认继承了过去，也不希望依赖模块构建结束后修改了当前模块的构建变量的值。因此我们希望构建工具能支持更好的模块化构建，即按照不同的构建目标闭包对应的构建变量。

[CMake](https://cmake.org/)从3.0版本开始支持模块化构建。它引入了target的概念，以及基于target建立起了构建上下文的可见性和传播控制机制，可以满足我们的上述需求。关于Modern CMake的用法和最佳实践，建议看看我的朋友尉刚强的这篇文章：[《Modern CMake最佳实践》](https://www.jianshu.com/p/8abf754654c4)。不过，在现实中很多项目在用CMake，但仍旧是按照CMake老的以directory为中心的方式使用，浪费了Modern Cmake的强大威力。



git submodule、git subtree或者git repo等是对基于源码的依赖管理的改进。这些方式可以做到将代码分开管理，在需要的时候再拽到一起。这种方式比上种方式的改进在于，虽然依旧是源码依赖，但是每个模块可以选择性的只拉取自己需要的源码，而不是所有代码。因此这种依赖管理方式比前一种更“精准”一些，但仍旧是“过度的”。依赖对方未必需要看到对方的源码细节，否则大家的构建可能就会耦合在一起，导致最终发布的时候仍旧需要一起发布，做不到独立发布。


想要做到独立发布，至少需要做到构建上的解耦。

基于构建的依赖管理。基于目录管理，基于target模块进行管理。不要将所有的模块的头文件路径都放到依赖里面。

基于构建的没有版本管理，和冲突解决能力。

总结：源码管理适和划分不稳定的情况，构建依赖管理适合独立发布。
